...
#'
#' @section Parameters:
#'   - \code{formula_sp}: A \code{\link[stats]{formula}} object specifying the 
#'       parametric component of the semiparametric model.
#'   - \code{lrnr_baseline}: A baseline learner for estimation of the 
#'       nonparametric component. This can be pooled or unpooled by specifying 
#'       \code{return_matrix_predictions}.
...
Lrnr_glm_semiparametric <- R6Class(
  ...
  private = list(
    ...
    .train = function(task) {
      ...
      if (!append_interaction_matrix && binary_interaction_variable) {
        ...
        beta <- suppressWarnings(coef(glm.fit(
          A * V, Y, offset = family$linkfun(Q0), intercept = F, 
        ...
        # project onto model
        beta <- suppressWarnings(coef(glm.fit(
          A * V, Q, offset = family$linkfun(Q0), intercept = F, 
          weights = task$weights, family = family
        )))
      }
    ...
    .predict = function(task) {
      ...
      binary <- fit_object$binary_interaction_variable
