...
Lrnr_bilstm <- R6Class(classname = "Lrnr_bilstm", inherit = Lrnr_base, portable = TRUE, class = TRUE,
                     public = list(
                     ...
                     private = list(
                       ...
                       .train = function(task) {
                         ...
                         #Build the model
                         model<-Sequential()
                         model$add(Bidirectional(LSTM(units=args$units, return_sequences=TRUE, input_shape = c(num_steps, num_features))))
                         model$add(TimeDistributed(Dense(args$dense)))
