...
## w/ xgboost, ***** seems to fail on travis ***** :
...
Sys.sleep(1); h2o::h2o.init(nthread = 1); Sys.sleep(1)
# ## some fits fail (response cannot be constant), in which case it engages the glm fall-back:
system.time(res_h2oGLM <- test_learner_dens(Lrnr_condensier, task, nbins = 3, max_n_bin = 50, bin_estimator = Lrnr_h2o_glm$new(family = "binomial")))
