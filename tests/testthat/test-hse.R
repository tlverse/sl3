context("test-hse.R -- Lrnr_density_hse")
...
# ggplot(pred_data, aes(x=x,y=y))+geom_tile(aes(fill=true_dens))+geom_point(data=data)
# ggplot(pred_data, aes(x=x,y=y))+geom_tile(aes(fill=true_dens))+geom_point(data=data)
# ggplot(pred_data, aes(x=(y-3*x),y=true_dens))+geom_line() + geom_point(aes(y=dens_preds),color="red") + xlim(-3,3) + theme_bw()
