true_obj_size=function(obj){length(serialize(obj, NULL))}
reduce_fit_test<-function(learner_fit){
  #given a learner fit, sequentially drop components from the internal fit object, keeping track of which components are needed for prediction
  ...
  for(component in components){
    ...
    try({reduced_predict=reduced$predict()  }, silent= TRUE)
