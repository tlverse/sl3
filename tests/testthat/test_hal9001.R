...
test_that("Lrnr_hal9001 works with stacking", {
  glm_learner <- Lrnr_glm$new()
  fglm_learner <- Lrnr_hal9001$new()
...
test_that("Lrnr_hal9001 works with quasibinomial and continuous outcomes in (0,1)", {
  ...
  fglm_learner <- Lrnr_hal9001$new(family = quasibinomial())
  fGLM_fit <- fglm_learner$train(task_01range)
  # print(fGLM_fit)

  fglm_learner <- Lrnr_hal9001$new(family = binomial())
  fGLM_fit <- fglm_learner$train(task_01range)
  # print(fGLM_fit)
})

test_that("Lrnr_hal9001 works with different families ('family = ...') and solvers ('method = ...')", {
  ...
  fglm_learner <- Lrnr_hal9001$new(family = binomial(), method = "eigen")
  fGLM_fit <- fglm_learner$train(task_bin)
  # print(fGLM_fit)

  fglm_learner <- Lrnr_hal9001$new(family = binomial(), method = "Cholesky")
  fGLM_fit <- fglm_learner$train(task_bin)
  # print(fGLM_fit)

  fglm_learner <- Lrnr_hal9001$new(family = binomial(), method = "qr")
  fGLM_fit <- fglm_learner$train(task_bin)
  # print(fGLM_fit)
})
...
test_that("When speedglm fails (singlular X) the fallback glm works", {
  ...
  fglm_lrnr <- Lrnr_hal9001$new(method = "Cholesky")$train(task_all)
