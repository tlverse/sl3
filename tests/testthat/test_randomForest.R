context("Test randomForest classification")
...
cpp <- cpp[1:150, ]
# make a fake categorical outcome
haz_quants <- quantile(cpp$haz, c(0.33, 0.67, 1))
cpp$multi_y <- cut(cpp$haz, c(-Inf, haz_quants), labels = FALSE)


cpp$binom_y <- cpp$haz>0
outcome <- "multi_y"
task <- sl3_Task$new(cpp, covariates = covars, outcome = outcome)

expect_equal(task$outcome_type, "categorical")
...
lrnr_mean_fit <- lrnr_mean$train(task)
p1=lrnr_mean_fit$predict()
lrnr_mean_fit$chain()
rf2 <- make_learner(Lrnr_randomForest,nonsense=2)
rf_fit <- rf$train(task)
p2=rf_fit$predict()
z=rf_fit$chain()
z$X
p2
rf_stack <- Stack$new(rf, xgb, lrnr_mean)
rf_stack_fit <- rf_stack$train(task)
mn_task <- rf_stack_fit$chain()
mn_task

mn_sl <- make_learner(Lrnr_sl, rf_stack, solnp_learner)
