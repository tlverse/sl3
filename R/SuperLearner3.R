...
convert_SL.library <- function(SL.library, family = "gaussian") {
  screeners <- sapply(SL.library, function(learner) {
                        ifelse(length(learner) == 1, "All", learner[[2]])
               })
  ...
  learner_objs <- lapply(unique_screeners, function(screener) {
    ...
    learner_objs <- lapply(learners_with_screener, function(learner) {
                             make_learner(Lrnr_pkg_SuperLearner,
                                          SL_wrapper = learner,
                                          family = family)
                    })
    learner_stack <- make_learner(Stack, learner_objs)
    if (screener != "All") {
      learner_screener <- make_learner(Lrnr_pkg_SuperLearner_screener,
                                       SL_wrapper = screener, family = family)
...
SuperLearner3 <- function (Y, X, newX = NULL, family = gaussian(), SL.library, 
  ...
  training_task <- sl3_Task$new(data = task_data, covariates <- covariates,
                                outcome = "Y", id = idvar, weights = weightvar)
  ...
  sl_learner <- Lrnr_sl$new(learner_objs, metalearner,
