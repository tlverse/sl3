...
#' @param trim if true, remove entries after failure time for each observation
#' @export
pooled_hazard_task <- function(task, trim = TRUE){
  ...
  underlying_data <- task$internal_data$raw_data
  ...
  set(underlying_data,j=id_name,value=id)
  ...
  # add new columns for indicator (new outcome) and bin index (additional covariate)
  new_columns <- repeated_task$add_columns(data.table(bin_number=bin_number, in_bin=in_bin))
  new_covariates <- c(task$nodes$covariates,"bin_number")
  hazard_task <- repeated_task$next_in_chain(column_names = new_columns, outcome="in_bin", covariates=new_covariates)
  if(!trim){
    return(hazard_task)
  }
