...
offset_task <- sl3_Task$new(data, covariates = Wnodes, outcome = Anode, offset = "offset")
...
glm_offset_fit <- glm(A~W1+W2+W3+W4, data, family=binomial(), offset=qlogis(data$offset))
...
test_learner_offset_support <- function(learner){
  ...
  # todo: verify something about training vs prediction offsets
  # todo: verify something about offset transformations
}

# instantiate these manually for now, because some have required args
# offset_learners <- c("Lrnr_glm", "Lrnr_glm_fast", "Lrnr_h2o_glm", "Lrnr_h2o_grid", 
#                      "Lrnr_optim", "Lrnr_solnp", "Lrnr_xgboost")
offset_learner_stack <- make_learner_stack(Lrnr_glm,
                                      Lrnr_glm_fast,
                                      Lrnr_mean,
                                      Lrnr_xgboost
                                      )
offset_learners <- offset_learner_stack$params$learners
lapply(offset_learners, test_learner_offset_support)
