...
test_that("fastGLM_Learner works with screener", {
    ...
    ## FAILS, because screener currently renames the covariates fglm_learner <- fastGLM_Learner$new(covariates =
    ## c('apgar1', 'meducyrs'), interactions = list(c('apgar1', 'meducyrs')))
    fglm_learner <- fastGLM_Learner$new()
