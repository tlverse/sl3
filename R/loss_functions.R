...
cv_risk <- function(learner, loss_fun, coefs = NULL) {
  ...
  loss_long <- melt(loss_dt, measure.vars=pred_cols, 
  ...
  loss_stats <- loss_by_id[,list(coefficients = NA_real_,
                                 risk=mean(loss, na.rm=TRUE),
                                 se=(1 / sqrt(.N)) *sd(loss)), by=list(learner)]
  ...
  loss_stats_fold <- loss_fold_stats[, list(fold_sd=sd(risk, na.rm=TRUE), 
